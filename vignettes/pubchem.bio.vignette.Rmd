...
###### the 'min.taxid.table.length' option
There are many species, fewer genera, fewer still family, order, etc.  If you plot the number of levels of these (not using the sub-species, sub-genera, etc) you can see a nice curve reflecting this.   

```{r import.and.plot.tax.image, eval = TRUE, fig.align="center", fig.cap="taxa counts by taxonomy level, specialized vs primary metabolite."}
knitr::include_graphics("min.taxid.table.length.graphic.jpeg")
```
The red curve shows the change in the number of taxa as we move from species through family, class, all the way to the root of all life.   A true primary metabolite should show the same curve, roughly, when looking at the distrubution of taxa it is mapped to.  We can see this with glucose (blue trace).   However, for bergaptol, a furanocoumarin specialized metabolite, you can see that there are two flat regions, one of which drops to zero (on the log scale).   That is, as you move through higher taxonomic levels, you will see that there becomes zero taxonomic diversity.   The min.taxid.table.length variable in this function enables you to control how lca is assigned. The taxid table lenght refers to the number of unique taxa values at a given taxonomic level.  In the red trace above - every taxonomic level has a unique value.  In the green however, there are two flat regions.  The largest flat region starts at 'class' and extends through 'root' - this region represents the fact that there is only one class (Magnoliopsida).  The smaller flat region indicates that this metabolite is found in only three families - Apiaceae, Rutaceae, and Moraceae - and their parent orders.  These flat regions indicate metabolites that are not as widely conserved. There are only 4 unique values in the green curve. In this example, if we set the min.taxid.table.length to 5, then an LCA would be assigned to bergaptol for EACH family.  that is three lowest common ancestors, preventing it from being assigned as a broadly distributed 'primary' metabolite, as would occur with the default value of '3'.  There is going to be a tradeoff - as you set this value higher, your metabolome for any given taxa will get smaller, but you will start to exclude metabolites which may plausibly be present in your sample.  The default value is designed to be inclusive - it is up to the user to be more restrictive as needed for that application.  
...
The databases you are building are stored in 'data.table' format.  It seemed overkill to build filtering functions around data.table's already  filtering functions, so instead, i will just include a few examples of how this filtering can be done.  

```{r filter.data.table, eval=FALSE}
## filter by monoisotopic molecular mass
pc.bio.sub <- pc.bio[pc.bio$monoisotopic.mass > 50 & pc.bio$monoisotopic.mass < 1200,]
range(pc.bio.sub$monoisotopic.mass)

## remove all rows that have a formula with a charge
charged <- grepl("+", pc.bio$formula, fixed = TRUE) | grepl("-", pc.bio$formula, fixed = TRUE)
pc.bio.sub <- pc.bio[!charged,]
...
export.CompoundDb(pc.bio.object = pc.bio.sub, path = "C:/Temp/20250703/")
```
...
library(pubchem.bio)
local.pubchem.directory <- "C:/Temp/20251217"  ## i suggest this naming scheme, but feel free to chose your own.
...
pubchem.bio::build.cid.lca(pc.directory = local.pubchem.directory, tax.sources =  "LOTUS - the natural products occurrence database", threads = 8, min.taxid.table.length = 3)
c <- Sys.time()
...
pubchem.bio::export.ComboundDb(pc.bio.object = pc.bio.sorghum, pc.directory = local.pubchem.directory, path = local.pubchem.directory, dbFile = 'sorghum.sql', organism = "Sorghum", get.synonyms = TRUE)
h <- Sys.time()
