...
By organizing taxonomy-metabolite data within the taxonomic hierarchy in cid-lca pairs (above), we can use some logical/reasoned steps to ensure that our taxonomy-specific metabolome database is as comprehensive as possible.   If we know that [Capsaicin](https://pubchem.ncbi.nlm.nih.gov/compound/Capsaicin) is found in _Capsicum annuum_ and _Capsicum frutescens_ (amongst others), once can reasonably infer that capsaicin _could_ be found in other _Capsicum_ species, such as _Capsicum cornutum_ even if there is no documented evidence of this in PubChem.  Therefore, if the only two know sources of Capsacin were the two Capsicum species, we can set the lowest common ancestor to _Capsicum_.  
